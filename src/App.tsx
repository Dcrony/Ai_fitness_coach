...
function App() {
  ...
  const drawSkeleton = (ctx: CanvasRenderingContext2D, landmarks: any[]) => {
    ...
    landmarks.forEach((landmark: any, index: number) => {
      ...
      // Color hips and knees specially for squat tracking
      if ([23, 24, 25, 26].includes(index)) {
        ctx.fillStyle = '#ff0066' // Pink for hips/knees
      } else {
        ctx.fillStyle = '#00d9ff' // Blue for others
      }
      ...
      // Draw index numbers for debugging
      ctx.fillStyle = '#fff'
      ctx.font = '12px Arial'
  ...
  const analyzeSquat = (landmarks: any[]) => {
    ...
    // Also check hip height relative to knees for better detection
    const hipHeight = (leftHip.y + rightHip.y) / 2
    const kneeHeight = (leftKnee.y + rightKnee.y) / 2
    
    // Debug info
    setDebugInfo(`Knee angle: ${Math.round(avgKneeAngle)}° | State: ${squatState}`)
    
    // IMPROVED SQUAT DETECTION LOGIC
    // Standing: knee angle > 160° (nearly straight)
    // Squatting: knee angle < 110° (bent)
    // Transition: 110°-160°
    ...
    if (avgKneeAngle < 110 && squatState !== 'down') {
      ...
      if (squatCount.current % 5 === 0) {
        ...
        const encouragements = [
          "Nice rep!", "Strong!", "Keep it up!", 
          "Powerful!", "Great form!", "Crushing it!",
          "One more!", "Perfect!"
        ]
  ...
  return (
    <div className="app">
      ...
      <div className="stats">
        ...
        <div className="stat feedback-box">
          <span className="label">Coach Says</span>
          <span className="value">{feedback}</span>
        </div>
      </div>

      {/* Debug info to help you see what AI sees */}
